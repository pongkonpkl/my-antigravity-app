...
# --- BACKEND (EMBEDDED API) ---
api_app = FastAPI(title="Coin PT Embedded API")
...
@api_app.post("/api/social")
def create_social_post(post: SocialPost, request: Request):
    if post.honeypot: raise HTTPException(status_code=400, detail="Bot activity detected")
    client_ip = request.client.host
    now = time.time()
    if client_ip in rate_limit_store:
        if now - rate_limit_store[client_ip] < COOLDOWN_SECONDS:
            raise HTTPException(status_code=429, detail="Rate limit exceeded")
    if not post.content.strip(): raise HTTPException(status_code=400, detail="Content required")
...
@api_app.delete("/api/social/{post_id}")
...
# Start API in background thread if not already running
if not any(t.name == "APIThread" for t in threading.enumerate()):
    threading.Thread(target=run_api, name="APIThread", daemon=True).start()
